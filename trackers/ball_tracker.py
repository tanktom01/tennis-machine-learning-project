...
class BallTracker:
    ...
    def interpolate_ball_position(self, ball_positions):
        ...
        )  # Capture the datapoints the tracker does capture

        # Interpolate
        df_ball_positions = df_ball_positions.interpolate()
        # To ensure the program can work even if it hasnt detected a value, we can back fill the position values
        df_ball_positions = df_ball_positions.bfill()
        # Return to orginal format
        ...
        for frame, ball_dict in zip(video_frames, ball_detections):
            ...
            for track_id, bbox in ball_dict.items():
                ...
                cv2.putText(
                    frame,
                    f"Ball ID: {track_id}",
                    (int(x1), int(y1 - 10)),  # Corrected this line
